...
public class RecordGeneratorUtils {
	...
	private RecordGeneratorUtils() throws RecordGeneratorException {
		try {
			femaleFirstNames = Files.readAllLines(new File("testdata/female_first_names.txt").toPath(),
		...
		try {
			maleFirstNames = Files.readAllLines(new File("testdata/male_first_names.txt").toPath(),
					Charset.defaultCharset());
		} catch (IOException e) {
		...
		try {
			lastNames = Files.readAllLines(new File("testdata/last_names.txt").toPath());
		} catch (IOException e) {
	...
	private static String[] adjectives = { "", "Great", "Beautiful", "Better", "Worse", "Gorgeous", "Terrific",
			"Fantastic", "Nebulous", "Colorful", "Terrible", "Natural", "Wild" };
