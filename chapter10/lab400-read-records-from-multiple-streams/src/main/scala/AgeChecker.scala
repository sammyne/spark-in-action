...
class AgeChecker extends ForeachWriter[Row] {
  ...
  def process(arg0: Row) {
    ...
    if (age < 13)
      log.debug(
        s"On stream #${streamId}: ${arg0.getString(0)} is a kid, they are ${age} yrs old.",
      )
    else if (age > 12 && age < 20)
      log.debug(
        s"On stream #${streamId}: ${arg0.getString(0)} is a teen, they are ${age} yrs old.",
      )
    else if (age > 64)
      log.debug(
        s"On stream #${streamId}: ${arg0.getString(0)} is a senior, they are ${age} yrs old.",
      )
  }
