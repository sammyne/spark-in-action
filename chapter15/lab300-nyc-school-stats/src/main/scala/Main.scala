...
object Main extends App {
  ...
  var masterDf =
    loadDataUsing2018Format(spark, "testdata/2018*.csv")

  masterDf = masterDf.unionByName(
    loadDataUsing2015Format(spark, "testdata/2015*.csv"),
  )
  ...
  log.debug(
    s"${year} was the year with most students, the district served ${max} students.",
  )
  ...
  absenteeRatioDf = absenteeRatioDf
    ...
    .agg(
      F.avg("enrolled").as("avg_enrolled"),
      F.avg("absent").as("avg_absent"),
    )
  ...
  /** Loads a data file matching the 2018 format, then prepares it.
    ...
    * @return
    */
  private def loadDataUsing2018Format(
      spark: SparkSession,
      fileNames: String*,
  ): DataFrame = {
  ...
  /** Load a data file matching the 2015 format.
    ...
    * @return
    */
  private def loadDataUsing2015Format(
      spark: SparkSession,
      fileNames: String*,
  ): DataFrame =
